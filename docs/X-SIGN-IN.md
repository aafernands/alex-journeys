# X (Twitter) reader sign-in — Alex Journeys
...
| `AUTH_URL` | **Set on Production** to `https://www.alexjourneys.com` (no path, no trailing slash). See below. |
...
3. App permissions: **Read** is enough. Do not switch the app to Read and Write. The authorize request uses the Auth.js Twitter default, `users.read tweet.read offline.access` (name, profile photo, a refresh token, and the scope X requires for the profile lookup). The site still does not post.
   ...
   Readers who already pressed Authorize under the old scope (`users.read` and `offline.access` only) need to authorize **once more** so X grants `tweet.read`. Use **Continue with X** again. If X does not show a new consent screen and sign-in still fails, revoke Alex Journeys in X settings (Settings → Security and account access → Apps and sessions) and try again.
...
5. Callback / Redirect URIs — add each exact URL. The one Auth.js uses in production is the **www** row. X compares it character for character (scheme, host, path).
   ...
   | Where | Callback |
   | --- | --- |
   | Production (required — this is the callback Auth.js advertises) | `https://www.alexjourneys.com/api/auth/callback/twitter` |
   | Production apex, only if that host can open `/login` without redirecting to www first | `https://alexjourneys.com/api/auth/callback/twitter` |
...
6. Website URL: `https://www.alexjourneys.com`
7. Optional but useful on the app settings: privacy `https://www.alexjourneys.com/privacy`, terms `https://www.alexjourneys.com/terms`.
...
| `InvalidCheck` | After Authorize, Auth.js could not read the state cookie (`state value could not be parsed`). The cookie was missing on the way back from x.com, or it was stored on www and the callback ran on the apex (or the reverse). Auth.js would otherwise show this as `Configuration`. | `/login` says the state cookie was lost. Stay on `https://www.alexjourneys.com`, in a normal window. Production state and PKCE cookies use `Domain=.alexjourneys.com`, `Secure`, `SameSite=Lax`, `Path=/`. Refresh and try Continue with X again. |
