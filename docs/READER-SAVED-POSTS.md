...
| Surface | Behavior |
| --- | --- |
| Header | Signed out: **Sign in** → `/login` on desktop. On a phone, Sign in is only in the menu (full-width **Sign in / Create account**), not the sticky bar. Signed in: avatar menu (photo + name on desktop; on a phone, the account row is in the menu) → Account dashboard, Logout; admins also see **Admin console** → `/cms`. The bottom tab bar stays Places / Stories / Guides / Saved. |
